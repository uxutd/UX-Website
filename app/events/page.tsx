...
const EventTitle: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <Typography sx={{ fontSize: '2.3rem', color: 'white', marginBottom: '0.2rem', fontFamily: 'Bricolage Grotesque', fontWeight: 'bold', textShadow: '0.1875rem 0.125rem 0.25rem #3E68FD' }}>
    {children}
  </Typography>
);

const EventDate: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <Typography variant="body2" sx={{ color: 'white', fontFamily: 'Bricolage Grotesque', fontSize: '1.1rem', fontWeight: 'bold', textShadow: '0.1875rem 0.125rem 0.25rem #3E68FD', marginTop: '0.001rem' }}>
    {children}
  </Typography>
);
...
export default function EventsPage() {
  ...
  return (
    ...
    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", gap: '1.5rem', padding: '1rem', marginTop: isMobile ? '48rem' : '57.2rem' }}>
      <Box sx={{ display: "flex", flexDirection: "column", alignItems: isMobile ? "center" : "flex-start", gap: '0.0625rem', position: "relative", marginLeft: isMobile ? 0 : '61.875rem' }}>
        <Typography variant="h2" gutterBottom sx={{ marginTop: isMobile ? 0 : '-3.75rem', fontFamily: 'Bricolage Grotesque', fontWeight: 'bold', color: '#3E68FD', textAlign: isMobile ? 'center' : 'left' }}>
          Previous Events
        </Typography>
        <img src="/smallsquigglyarrow.png" alt="Arrow" style={{ width: isMobile ? '10rem' : '12.5rem', height: 'auto', position: 'absolute', right: isMobile ? '-3.5rem' : '-5.3125rem', top: isMobile ? '4rem' : '0.3125rem', zIndex: 1 }} />
      </Box>
      <Box sx={{ display: "flex", flexWrap: "wrap", gap: '1.5rem', justifyContent: "center", maxWidth: '75rem' }}>
        {visibleEvents.map((event, index) => (
          <Box key={index} sx={{ width: '23.4375rem', height: '23.4375rem', borderRadius: '3rem', overflow: "hidden", transition: "transform 0.3s", boxShadow: 3, zIndex: 0, '&:hover': { transform: "scale(1.05)", boxShadow: 6 } }}>
            <Card sx={{ width: "100%", height: "100%", borderRadius: '3rem', position: 'relative' }}>
              <CardMedia
                component="img"
                height="100%"
                image={event.image}
                alt={event.alt}
                sx={{ objectFit: 'cover', width: '100%', height: '100%', ...event.sx }}
              />
              <Box sx={{ position: 'absolute', top: 0, width: '100%', height: '30%', background: 'linear-gradient(to bottom, rgba(0,0,0,0.7), rgba(0,0,0,0))', zIndex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', padding: '1rem' }}>
                <EventTitle>
                  {event.title}
                </EventTitle>
                <EventDate>
      ...
      <Box sx={{ marginTop: '1.5rem' }}>
        <button onClick={() => setShowMore(!showMore)} style={{ padding: '0.25rem 0.5rem', fontSize: '1.75rem', cursor: 'pointer', fontFamily: 'Bricolage Grotesque', fontWeight: 'bold', color: '#3E68FD', border: '0.1875rem solid #3E68FD', borderRadius: '0.5rem' }}>
