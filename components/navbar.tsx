        ...
        <NavbarContent className="hidden lg:flex justify-center basis-full">
          <ul className="flex gap-4 justify-center items-center mx-auto w-full max-w-md">
            {siteConfig.navMenuItems.map((item) => (
              <NavbarItem key={item.href} className="relative w-full text-center group">
                <motion.div
              ...
              <NavbarMenuItem key={`${item.label}-${index}`}>
                ...
                {item.sublinks && item.sublinks.length > 0 && (
                  <div className="ml-4 flex flex-col gap-2 pl-4 border-l border-gray-300 mt-1">
                    {item.sublinks.map((sublink, subIndex) => (
                      <Link key={`${sublink.label}-${subIndex}`} href={sublink.href} size="sm" className="hover:text-primary">
                        {sublink.label}
                      </Link>
                    ))}
                  </div>
